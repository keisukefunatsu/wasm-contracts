...
export default class Constructors {
	...
	*/
   	async "new" (
		...
		__options ? : ConstructorOptions,
   		...
   		const __contract = JSON.parse(Files.readFileSync("./artifacts/psp22.contract").toString());
		const code = new CodePromise(this.nativeAPI, __contract, __contract.source.wasm);
		const gasLimit = (await _genValidGasLimitAndValue(this.nativeAPI, __options)).gasLimit as WeightV2;

		const storageDepositLimit = __options?.storageDepositLimit;
			const tx = code.tx["new"]!({ gasLimit, storageDepositLimit, value: __options?.value }, name, symbol, decimals, totalSupply);
			let response;
	...
	*/
   	async "newNoInitialSupply" (
		...
		const storageDepositLimit = __options?.storageDepositLimit;
			const tx = code.tx["newNoInitialSupply"]!({ gasLimit, storageDepositLimit, value: __options?.value }, name, symbol, decimals);
			let response;
